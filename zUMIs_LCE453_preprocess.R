...
# William Ho (based on Peter Hickey written on 2020-11-05)
...
# Construct NN197 sample sheet -------------------------------------------------

file_nn197 <- here(
  "data",
  "sample_sheets",
  "JN_DZ_C051 NN197_SarahBest_Jin_SeqPrimer layout_Oct20.xlsx")
...
sample_sheet_nn197 <- sample_sheet_nn197 %>%
  mutate_at(facs_data_idx, as.numeric)
# Tidy up names.
sample_sheet_nn197 <- bind_cols(
  clean_names(sample_sheet_nn197[, -facs_data_idx]),
  clean_names(sample_sheet_nn197[, facs_data_idx], case = "parsed"))
...
sample_sheet_nn197 <- bind_cols(
  ...
  as.data.frame(
    lapply(markers, function(marker) {
      tmp <- as.matrix(
        cbind(sample_sheet_nn197[, grep(marker, colnames(sample_sheet_nn197))]))
      stopifnot(all(rowSums(!is.na(tmp)) <= 1))
      rowSums(tmp, na.rm = TRUE)
    })))

# Remove empty rows/columns.
sample_sheet_nn197 <- remove_empty(
  sample_sheet_nn197,
  which = c("rows", "cols"))
...
sample_sheet_nn197 <- sample_sheet_nn197 %>%
  mutate(
    ...
    sequencing_run = "NN197") %>%
  rename(patient_id = id) %>%
  arrange(plate_number, well_position)

# Construct data frame linking plates to donors --------------------------------

# NOTE: This table does not contain any of the patients from NN197. Instead, 
#       this information is included in the NN197 sample sheet.
...
### WH: skip this chunk as only deal with LCE453 of NN197 
# 
...
r1_fq <- c(
  ...
  grep(
    ...
    x = list.files(
      path = here("extdata", "NN197", "merged"),
      full.names = TRUE,
